...
object BuiltinFunctions
{
  ...
  val builtinFunctions = Map[BuiltinFunction.Value, Function](
      ...
      BuiltinFunction.ZNot -> new Function(1) {
        override def applyS[T, U, V, E](argValues: Seq[Value[T, U, V]])(env: E)(implicit eval: Evaluator[SimpleTerm[T, U], E, Value[T, U, V]]) = argValues match {
          case Seq(BooleanValue(x)) => (env, BooleanValue(!x))
          case _                    => (env, illegalAppNoValue)
        }
      },
      BuiltinFunction.ZAnd -> new Function(2) {
        override def applyS[T, U, V, E](argValues: Seq[Value[T, U, V]])(env: E)(implicit eval: Evaluator[SimpleTerm[T, U], E, Value[T, U, V]]) =
          argValues match {
