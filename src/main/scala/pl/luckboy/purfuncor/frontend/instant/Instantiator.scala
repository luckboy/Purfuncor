...
object Instantiator 
{
  ...
  def addInstancesFromTreeInfoS[T, U, V, W, X, Y, E](treeInfo: typer.TreeInfo[T, U, V])(env: E)(implicit polyFunInstantiator: PolyFunInstantiator[U, X, V, TypeLambdaInfo[W, Y], E], instTreeExtractor: InstantiationTreeInfoExtractor[T, U, frontend.Instance[U], SelectConstructInstance[X, TypeLambdaInfo[W, Y]]]) = {
    val insts = instTreeExtractor.instancesFromTreeInfo(treeInfo.treeInfo)
    val selectConstructInsts = instTreeExtractor.selectConstructInstancesFromTreeInfo(treeInfo.treeInfo)
