  ...
  def logicalTypeValueTermS[U, V, W, E](env: E)(implicit eval: Evaluator[TypeSimpleTerm[U, V], E, TypeValue[T, U, V, W]], envSt: TypeEnvironmentState[E, T, TypeValue[T, U, V, W]]) =
    this match {
      case GlobalTypeApp(loc, args, sym) =>
        val (env2, hasApplyingTypeComb) = envSt.hasApplyingTypeCombinator(loc)(env)
        if(!hasApplyingTypeComb) {
...
{
  lazy val supertypeInfo = LogicalTypeValueTermInfo.fromTypeValueNodeWithArgs(conjNode, args, true)
  ...
  def normalizedTypeValueNodeForChecking(isSupertype: Boolean, canExpandGlobalType: Boolean) =
    LogicalTypeValueTerm(conjNode.normalizedTypeValueNodeForChecking(isSupertype, canExpandGlobalType), args + (BuiltinTypeIdentity(TypeBuiltinFunction.Any, Nil) -> Nil) + (BuiltinTypeIdentity(TypeBuiltinFunction.Nothing, Nil) -> Nil))
