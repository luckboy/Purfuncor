...
"""
    ...
    it should "unify the two group types" in {
      // Unifies T #Boolean #Float
      // with    U #Boolean #Flaot
      // for type T t1 t2 = V t1 t2 and type U t1 t2 = V t1 t2 and grouptype 2 V.
      val s = """
type T t1 t2 = V t1 t2
type U t1 t2 = V t1 t2
grouptype 2 V
f = construct 0: T #Boolean #Float
g (x: U #Boolean #Float) = x
h = g f
"""
      inside(resolver.Resolver.transformString(s)(NameTree.empty).flatMap(f)) {
        case Success(tree) =>
...
""")(NameTree.empty)(f).run(emptyEnv)
      ...
      // i
      def testILambdaInfos(param111: Int, param121: Int, param122: Int)
      {
...
""")(NameTree.empty)(f).run(emptyEnv)
      ...
      // h
      def testCombs(param111: Int, param112: Int, param312: Int, param313: Int, combTypeParams1: Map[Int, Int])
      {
        // i
        inside(enval.globalVarTypeFromEnvironment(env)(GlobalSymbol(NonEmptyList("i")))) {
...
""")(NameTree.empty)(f).run(emptyEnv)
    ...
    it should "complain on the instantiation of the parameters of the defined types for the integer function" in {
      val (env, res) = Typer.inferTypesFromTreeString("""
f = #iAdd: \t1 => ##-> t1 (##-> t1 t1)
""")(NameTree.empty)(f).run(emptyEnv)
     inside(res) {
